...
import { useState, useEffect } from "react";
import React from "react"; // needed because you use <React.Fragment>
import {
  FiSearch,
  FiFilter,
  FiChevronRight,
...
const AdminAppointmentsPage = () => {
  ...
  // --- service editor state (used by openServiceEditor, etc.)
  const [serviceEditOpenForId, setServiceEditOpenForId] = useState(null);
  const [serviceEditLoading, setServiceEditLoading] = useState(false);
  const [activeServices, setActiveServices] = useState([]);
  const [pickedServiceIds, setPickedServiceIds] = useState([]);

  // --- lightweight edit state used by updateStatus fallbacks
  const [editingId, setEditingId] = useState(null);
  const [editStatus, setEditStatus] = useState("");
  ...
  useEffect(() => {
    ...
    const fetchStaff = async () => {
      ...
      const response = await fetch(`${ADMIN_API_BASE_URL}/staffview`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  //     return a.id !== editingId && a.date === inputDate && a.time === inputTime;
  //   });
  ...
  const updateStatus = async (
    ...
    try {
      const response = await fetch(
        `${ADMIN_API_BASE_URL}/update-appointment-status`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleApptSaved = (updated) => {
    setShowEditModal(false);
    setEditingAppt(null);

    const done = String(updated.status || "").toLowerCase() === "completed";
    setAppointments((prev) =>
      done
        ? prev.filter((a) => a.id !== updated.id) // drop completed rows from this view
  ...
  const handleAssignmentComplete = async (appointmentId, staffId) => {
    try {
      ...
      const response = await fetch(`${ADMIN_API_BASE_URL}/assign-staff`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const filteredAppointments = appointments.filter((appt) => {
    ...
    const idText = String(appt.id);
    const idMatch =
      idText.toLowerCase().includes(q) ||
      `no-${idText}`.toLowerCase().includes(q);

    const clientMatch = (appt.client || "").toLowerCase().includes(q);

    const stylistName = appt.staffId ? getStaffNameById(appt.staffId) : "";
    const stylistMatch = (stylistName || "").toLowerCase().includes(q);

    return statusOk && (idMatch || clientMatch || stylistMatch);
  });
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by Id, Client and Staff....."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
                    ...
                    return (
                      <React.Fragment key={appt.id}>
                        ...
                        >
                          ...
                          <td className="px-2 py-3 align-middle border-b border-gray-800 whitespace-nowrap">
                        ...
                        {appt.isCustom && expanded && (
                          <>
                            ...
                            <tr className="bg-indigo-900/60 ">
                              {/* caret, client */}
                              {/* <td className="border-b border-gray-800 px-2 py-2 bg-yellow-500" ></td> */}
                              <td className=" px-2 py-2"></td>
                              <td className=" px-2 py-2"></td>{" "}
                              {/* NEW for Apt-ID */}
                              {/* 1) No (colspan 2) */}
                              <td
                                className=" px-3 py-2 text-sm text-indigo-200/90 text-left"
                                colSpan={3}
                              >
                                No
                              </td>
                              {/* 2) Service (colspan 3) */}
                              <td
                                className=" px-3 py-2 text-sm text-indigo-100 text-center"
                                colSpan={3}
                              >
                                Service
                              </td>
                              ...
                              <td className=" px-2 py-2"></td>
                              <td
                                className=" px-3 py-2 text-sm font-semibold text-indigo-200 text-center"
                                colSpan={3}
                              >
                                Amount (LKR)
                              </td>
                            ...
                            {(appt.items && appt.items.length
                              ...
                              <tr
                                key={`sub-${appt.id}-${idx}`}
                                className="bg-indigo-900/50 transition "
                              >
                                {/* caret, id, client placeholders */}
                                ...
                                <td className=" px-2 py-2"></td>
                                <td
                                  className=" px-3 py-2 text-sm text-indigo-100 text-center tabular-nums"
                                  colSpan={3}
                                >
                            ...
                            <tr className="bg-indigo-900/70">
                              ...
                              {/* 'Total' (colspan 5) */}
                              <td
                                className=" px-3 py-2 font-semibold text-indigo-100 text-left"
                                colSpan={5}
                              >
                                Total
                              </td>
                              {/* amount (colspan 2) */}
                              <td
                                className="px-3 py-2 font-semibold text-indigo-50 text-center tabular-nums"
                                colSpan={3}
                              >
        ...
        {showEditModal && editingAppt && (
          <EditAppointmentModal
            ...
            adminApiBase={ADMIN_API_BASE_URL}
            appt={editingAppt}
            onClose={() => {
              setShowEditModal(false);
              setEditingAppt(null);
